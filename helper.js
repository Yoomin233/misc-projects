function Listener(evtName) {
  if (!evtName) throw new Error("Must specify a event name!");
...
Listener.prototype.trigger = function() {
  if (!this.callBacks || (this.callBacks && !this.callBacks.length)) return;
...
Listener.prototype.removeEventListner = function(evtName, func) {
  ...
  if (funcIndex !== -1) this.callBacks.splice(funcIndex, 1);
};
